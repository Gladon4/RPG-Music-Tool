...
class SFX_Tab():
	...
	def create(self, new=False):
		...
		self.label_title_paths = Label(self.frame, text="SFX Directories", font=("Helvetica",20), bg=settings["bg_color"], fg=settings["txt_color"])
		...
		self.add_button = Button(self.page_navigation_frame, text="Add SFX Directory", command=self.__sfx_path_picker, bg=settings["bg_color"], fg=settings["txt_color"], activebackground=settings["button_hov_color"])
		...
		if (self.page_number + 1) * self.paths_per_page >= len(self.set_manager.sfx_paths):
	...
	def __create_list(self):
		...
		REMAING_PATHS = len(self.set_manager.sfx_paths) - per_page * page
		for i in range(min(per_page, REMAING_PATHS)):
			index_with_offset = per_page * page + i
			path = self.set_manager.sfx_paths[index_with_offset]
	...
	def __previous_page(self):
		...
		if self.page_number == 0:
			self.previous_page_button.config(state="disabled")
		if not (self.page_number + 1)* self.paths_per_page >= len(self.set_manager.sfx_paths):
	...
	def __sfx_path_picker(self):
		...
		dirs = tkfilebrowser.askopendirnames(title="Select your SFX Directories", initialdir=initialdir, okbuttontext="Select")
		i = len(self.set_manager.sfx_paths)
		...
		for dir in dirs:
			...
			path_label.grid(row=i, column=1)
			self.paths[i] = [path_delete_button, path_label]
			self.set_manager.sfx_paths += [dir+"/"]
			i += 1
	...
	def __delete(self, index):
		...
		self.set_manager.sfx_paths = self.set_manager.sfx_paths[:index] + self.set_manager.sfx_paths[index+1:]
