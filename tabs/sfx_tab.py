...
class SFXTab:
    ...
    def create(self, new=False):
        ...
        self.page_navigation_frame = Frame(
            self.frame, bg=settings["bg_color"], width=1000
        )
        ...
        self.settings_button = Button(
            ...
            command=lambda tab="settings": self.__select_tab(tab),
    ...
    def __sfx_path_picker(self):
        ...
        for dir in dirs:
            path_delete_button = Button(
                self.list_frame,
                image=self.delete_image,
                command=lambda i=i: self.__delete(i),
                activebackground=self.set_manager.settings["button_hov_color"],
                bg=self.set_manager.settings["button_bg_color"],
            )
            ...
            path_label = Label(
                ...
                text=dir,
                font=("Helvetica", 12),
                bg=self.set_manager.settings["sec_bg_color"],
                fg=self.set_manager.settings["txt_color"],
            )
            path_label.grid(row=i, column=1)
            self.paths[i] = [path_delete_button, path_label]
    ...
    def __delete(self, index):
        ...
        self.set_manager.sfx_paths = (
            self.set_manager.sfx_paths[:index] + self.set_manager.sfx_paths[index + 1 :]
        )
    ...
    def __load_imgs(self):
        if getattr(sys, "frozen", False):
            ...
            self.delete_image = PhotoImage(
                file=os.path.join(sys._MEIPASS, "img/delete_img.png")
            )
